...
export function Shop({ coins, onUpdate }: Props) {
  ...
  const load = () => fetch('/api/furniture').then(r => r.json()).then(setItems)
  useEffect(() => { load() }, [])

  async function buy(id: number) {
    const res = await fetch(`/api/furniture/${id}/buy`, { method: 'POST' })
    setItems(await res.json())
    onUpdate()
  }

  async function toggle(id: number) {
    const res = await fetch(`/api/furniture/${id}/toggle`, { method: 'POST' })
    setItems(await res.json())
    onUpdate()
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {totalBonus > 0 && (
        <div className="pixel-box" style={{ padding: '12px 16px', textAlign: 'center' }}>
          <div style={{ fontFamily: 'var(--pixel-font)', fontSize: 10, color: 'var(--green)' }}>
            ...
            return (
              <div key={item.id} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div>
                    <div style={{ fontFamily: 'var(--pixel-font)', fontSize: 10, color: item.equipped ? 'var(--green)' : 'var(--text)' }}>
