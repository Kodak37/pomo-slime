...
- **スライムの5段階状態（CSSアニメーション）**:
  - `dying`（元気がない）: ほぼ水、ぺちゃんこ、ほとんど動かない
  - `hungry`（ちょっと元気がない）: ぐったり、のろい動き
  - `normal`（普通）: 通常のブロブ形状、穏やかな揺れ
  - `slightly_happy`（ちょっと元気）: 軽く跳ねる、ハリが見える
  - `happy`（元気）: 大きく跳ねる、弾力感MAX、高速アニメーション
